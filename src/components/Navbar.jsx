...
export default function Navbar({ settings }) {
  ...
  return (
    <nav className="bg-white border-b border-stone-200 sticky top-0 z-50 shadow-sm" style={{ borderBottomColor: s.colors?.primary + '20' }}>
      <div className="max-w-7xl mx-auto px-2 sm:px-6 lg:px-8">
        <div className="flex justify-between py-3 items-center gap-2">

          {/* Logo */}
          <div className="flex items-center gap-2 select-none">
            <span className={n.logoEmojiSize}>{s.logoEmoji}</span>
            <div className="flex flex-col">
              <div className={`${n.logoNameSize} font-bold text-stone-900 leading-none tracking-tight`}>
                {s.name}
              </div>
              <div className={`${n.logoTaglineSize} text-stone-500 font-medium`}>
                {s.tagline}
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-col items-end gap-0.5 shrink-0">
            {s.whatsappNo && (
              <a
                href={`https://wa.me/${s.whatsappNo.replace(/[^0-9]/g, '')}`}
                target="_blank"
                rel="noopener noreferrer"
                className="flex items-center gap-1.5 px-2 py-1 rounded text-white transition-colors"
                style={{ backgroundColor: s.colors?.primary || '#1c1917' }}
            ...
            {s.address && (
              <a 
                ...
                target="_blank" 
                rel="noopener noreferrer"
                className="text-[9px] text-stone-500 text-right hover:text-stone-900 hover:underline transition-colors"
