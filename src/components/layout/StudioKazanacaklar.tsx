...
interface StudioKazanacaklarProps {
  header: string;
  items: string[];
  website: string;
  presetClass: string;
...
export default function StudioKazanacaklar({
  header,
  items,
  website,
  presetClass,
            ...
            >
              <div className="shrink-0">
                <div className={`w-14 h-14 rounded-2xl flex items-center justify-center ${isLight ? 'bg-stone-200 text-stone-900 border border-stone-300' : 'bg-white/[0.03] border border-white/[0.06]'}`}>
                  <Lucide.Check className={isLight ? 'text-stone-850' : 'text-emerald-400'} size={24} />
                </div>
              </div>
              <div className="flex-1 space-y-1 text-left">
                <p className={`text-[19px] font-medium leading-relaxed ${isLight ? 'text-stone-600' : 'text-stone-300'}`}>
