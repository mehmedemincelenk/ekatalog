            ...
            {storyPages.map((page, pageIdx) => (
              ...
              >
                  ...
                  {page.products.map((product) => {
                    return (
                      ...
                      >
                        {/* Image Cell */}
                        <div
                          className={`w-12 h-12 rounded-lg overflow-hidden border shadow-sm bg-white shrink-0 ${storyTheme === 'DARK' ? 'border-stone-800' : 'border-white'}`}
                        >
                          <img
                        ...
                        <div className="flex-1 flex items-center justify-between min-w-0 pl-3 gap-2">
                          <div className="flex-1 min-w-0 story-text-container">
                            <h4
                              className={`text-[10px] font-black m-0 p-0 overflow-hidden whitespace-nowrap ${storyTheme === 'DARK' ? 'text-white' : 'text-stone-900'}`}
                              style={{ lineHeight: '1.2' }}
                            >
                              {product.name.toLocaleUpperCase('tr-TR')}
                            </h4>
                            <p
                              className={`text-[8px] font-medium m-0 p-0 overflow-hidden whitespace-nowrap opacity-50 ${storyTheme === 'DARK' ? 'text-stone-300' : 'text-stone-500'}`}
                          ...
                          <div className="shrink-0 text-right">
                            <span
                              className={`text-[11px] font-black ${storyTheme === 'DARK' ? 'text-white' : 'text-stone-900'}`}
                ...
                <div className="mt-4 flex flex-col items-center gap-2 shrink-0">
                  <p
                    className={`text-[8px] font-black lowercase tracking-[0.2em] opacity-40 ${storyTheme === 'DARK' ? 'text-white' : 'text-stone-900'}`}
                  >
                    {storeUrl}
                  </p>
                  {settings?.logoUrl && (
