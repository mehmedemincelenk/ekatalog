...
- **Key Actions:**
    ...
    - **Fallback Category Deletion PIN Protection:** Updated `CategoryHeader.tsx` to require the user to input the active admin PIN when attempting to delete the high-risk "Genel" category (since this permanently deletes all products under it). The "SİL" button is disabled until a PIN is entered, and deletes are only executed if the PIN matches the session `adminPin`.
    - **Bulk Operations PIN Protection:** Integrated a mandatory administrator PIN confirmation input in `AdminOperationsModal.tsx` on the final Step 3 (ActionDeskScreen) review for all 4 bulk system actions (Price, Stock, Archive, Delete). The execution button is disabled until the PIN is supplied, and action is only dispatched if it matches the session `adminPin`.
    ...
    - **Dynamic Logo/Name Anim Toggling:** Modified `AddReferenceModal.tsx` to conditionally hide/show the logo upload area and the text name input field using smooth Framer Motion height/opacity animations (`AnimatePresence`). Selecting a logo collapses the text name field, while typing a name collapses the logo upload area.
...
- **Wizard UX Standards**: 
    ...
    - Final confirmation buttons must follow the 'TAMAM' (Primary) vs 'ANLADIM' (Secondary) pattern.
